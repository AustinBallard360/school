...
{
	struct patientList* listx;
	listx  = malloc(sizeof(struct patientList)); // make new list object and allocate memory
	struct patient* p;
	//p = listx->patient; // p is now our patient node
		p = malloc(sizeof(struct patient)); // make memory space for our new patient
		p->roomNumber = roomNumInput; // set room number
		strcpy(p->name , patientNameInput); // set name
		p->doctors = NULL ; // set doctor // this might come up later idk.

	listx->patient = p;
	listx->next = list; // point our lists next towards head.
	list = listx; // head is now listx
...
{
	if(tempList!= NULL){ // stopping condition
		displayPatientList(tempList->next); // recursive call (m size soultion)
		struct patient* p = tempList->patient; // its a get a pointer for the inner patient node
		printf("\nPatient name: %s \n" , p->name); // print name
		printf("Room number: %d \n", p->roomNumber); // print room number
	}
...
struct patient* searchPatient(char* patientNameInput)
{
	...
	struct patient *p; // patient pointer
	while (tempList != NULL){ // loop thru the list 
		p = tempList->patient; // access inner patient node
		if(strcmp(patientNameInput , p->name ) == 0){ // compare patient name actual vs input  // 0 = same strings
			return p; // if identical, return patient node
		}else{ // if no match, move thru list
			tempList = tempList->next; // iterate
		}// end else
...
// Parse the list to locate the patient and delete that 'patient' node. // THIS IS DECIEVING, WE NEED TO DELETE A PATIENTLIST NODE, not just patient node!!
...
{	
	//tempList is node 1, tempNext is node 2 , tempNext->next is node 3
	struct patientList* tempList = list;	// work on a copy of 'list'
	struct patientList* tempNext = list->next; // pointer to the "next" patientList node.
	struct doctor* doc = NULL; // create doctor pointers for iteration & deletion
	struct doctor* docOld = NULL; // pointer for deletion / iterate

	if(strcmp(tempList->patient->name , patientNameInput) == 0){ // case for first object is the one we want to delete
		// GET ALL THE DOCTORS !!!!
		doc = tempList->patient->doctors ; // doctor node pointer
		while(doc != NULL){ // while real doctor nodes
			docOld = doc; // pointer to doc to delete
			doc = doc->next; // pointer to next doc
			free(docOld); // delete previous
		} // end doctors deletion
		free(tempList->patient); // delete patient node
		free(tempList); // delete head.
		list = tempNext; // head is now next node
	}

	while(tempNext != NULL){
		if(strcmp(tempNext->patient->name , patientNameInput) == 0){ // if the next patient is the one to be deleted.
			tempList->next = tempNext->next ; // point the first.next to third ( so we can delete 2nd node)
			doc = tempNext->patient->doctors ; // doctor node pointer
			while(doc != NULL){ // while real doctor nodes
				docOld = doc; // get current doctor pointer for deletion
				doc = doc->next; // get next doctor pointer before deletion
				free(docOld); // delete current
		}
			free(tempNext->patient); // delete patient node
			free(tempNext); // delete patientList node
			break; // we are done.
		}// end if match
		// increment thru the list
		tempList = tempNext;
...
{	
	// i never have done tail insert before so i will practice that.
	struct patientList* tempList = list;		// work on a copy of 'list'
	struct doctor* docList = NULL; // create doctor iterator
	struct doctor* newDoc; // create new doctor pointer
	while(tempList != NULL){ // loop thru patientList
		if(strcmp(tempList->patient->name , patientNameInput) == 0){ // if we find the desired patient.
			docList = tempList->patient->doctors; // make doctor pointer to the right place.
			break; // we are done here.
		}
		tempList = tempList->next; // move thru loop.
	} // end patientList while loop

	if(docList == NULL){ // no existing doctors
		docList = malloc(sizeof(struct doctor)); // assign memory to new doctor
		strcpy(docList->name , doctorNameInput); // give doctor name
		docList->next = NULL ; // add at end of list, no NEXT
		tempList->patient->doctors = docList;

	} // end no doctors case 
	else{ printf("OTHER THING GO !!! \n");
		while (docList->next != NULL){ // loop to end of doctors for insert @ tail.
			docList = docList->next; // move to end of list
		} // end while
		newDoc = malloc(sizeof(struct doctor)); // assign memeory for doctor.
		newDoc->next = NULL; // assign next ( this is last node) ( we are insert @ tail)
		strcpy(newDoc->name , doctorNameInput); // assign name;
		docList->next =newDoc; // point our old tail to our new tail.
	}
}
...
char* lastDoctor(char* patientNameInput)
{
	...
	struct patientList* tempList = list;		// work on a copy of 'list'
	struct doctor* docList = NULL; // create doctor iterator
	// copy find patient bit from addDoctor()
	while(tempList != NULL){ // loop thru patientList
		if(strcmp(tempList->patient->name , patientNameInput) == 0){ // if we find the desired patient.
			docList = tempList->patient->doctors; // make doctor pointer to the right place.
			break; // we are done here.
	...
	while(docList->next != NULL){ // find the last element
		docList = docList->next; // move to tail // docList is now tail
	} 

	return docList->name ; // idk if we can return it this way? is this a char pointer? who knows!
}
...
{ 
	if(tempList!= NULL){ // stopping condition
		displayPatientAndDoctorList(tempList->next); // recursive call (m size soultion)
		struct patient* p = tempList->patient; // get a pointer for the inner patient node
		struct doctor* doc = p->doctors ; // get pointer for doctor node
		printf("\nPatient name: %s \n" , p->name); // print name
		printf("Room number: %d \n", p->roomNumber); // print room number
		printf("Doctors: "); if(doc == NULL){printf(" \n"); } // if theres no doctors, newline for nice format.
		while(doc != NULL){ // loop thru multiple doctors if available.
			printf("%s \n", doc->name); // print doctor name
			doc = doc->next; // iterate thru doctors
		}
	}
}
